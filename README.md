...
以下模型已在本仓库当前版本中完成“真实工具调用”验证（可实际修改表格，不是仅文本回复）：

- **腾讯混元**：`hunyuan-turbos-latest`
- **Ollama 本地模型**：`qwen2.5:7b`、`qwen2.5-coder:latest`、`qwen2.5:3b`（复杂任务稳定性略低于 7b）

不推荐作为表格工具主力的模型（在部分环境下会降级为纯聊天，无法执行工具）：

- `chevalblanc/gpt-4o-mini:latest`（实测出现 tool 不支持回退）

为什么单独列这段：不同本地模型对工具调用协议支持差异较大；提前给出实测结果，能减少“模型能聊天但不改表格”的排障成本。
